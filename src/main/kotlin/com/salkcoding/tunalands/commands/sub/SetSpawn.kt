            ...
            if (player != null) {
                ...
                if (lands != null) {
                    ...
                    if (player.hasNotEnoughMoney(price)) {
                        val delta = (price - economy.getBalance(player)).round(2)
                    ...
                    when (data.rank) {
                        Rank.OWNER -> {
                            when (args[0]) {
                                "main" -> lands.memberSpawn = player.location.toCenterLocation()
                                "visitor" -> lands.visitorSpawn = player.location.toCenterLocation()
                                else -> return false
                            }
